class DeliveriesController < ApplicationController
    ...
    def index
        ...
        if is_logged_in?(session)        # Checks if user is logged in
            if (current_user(session) == find_user.id)  #  Makes sure you are looking at only your deliveries
                @user = find_user  # Gets the whatever-its-called variable set up for the view
    ...
    def new
        ...
        if (isGuest(session))
            ...
            if (get_user != current_user(session))
